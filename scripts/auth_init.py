...
def client_app(identifier: int) -> Application:
    return Application(
        display_name=f"Moneta Sample App {identifier}",
        sign_in_audience="AzureADMyOrg",
        web=WebApplication(
            redirect_uris=["http://localhost:8501/.auth/login/aad/callback"],
...
def client_userflow(identifier: int):
    ...
    return ExternalUsersSelfServiceSignUpEventsFlow(
        display_name=f"Moneta Sample User Flow {identifier}",
        description=f"Moneta Sample User Flow {identifier}",
        priority=500,
