...
from sk.aifoundry.assistant_content_generation import create_chat_message, generate_message_content

@experimental_class
class OpenAIAssistantChannel(AgentChannel):
    """OpenAI Assistant Channel."""

    def __init__(self, project_client: AIProjectClient, thread_id: str) -> None:
    ...
    async def receive(self, history: list["ChatMessageContent"]) -> None:
        ...
        for message in history:
            if any(isinstance(item, FunctionCallContent) for item in message.items):
                continue
            await create_chat_message(self.project_client, self.thread_id, message)
    ...
    async def get_history(self) -> AsyncIterable["ChatMessageContent"]:
        ...
        """
        thread_messages = self.project_client.agents.list_messages(
            thread_id=self.thread_id, limit=100, order="desc"
        )
    ...
    async def reset(self) -> None:
        ...
        try:
            ...
            self.project_client.agents.delete_thread(thread_id=self.thread_id)
